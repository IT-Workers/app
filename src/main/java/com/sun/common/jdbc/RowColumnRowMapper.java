 ...
 */
public class RowColumnRowMapper<T> implements RowMapper<T> {

	private Class<?> requiredType;


	/**
	 * Create a new {@code ResultRowMapper} for bean-style configuration.
	 * @see #setRequiredType
	 */
	public RowColumnRowMapper() {
	}

	/**
	 * Create a new {@code ResultRowMapper}.
	 * <p>Consider using the {@link #newInstance} factory method instead,
	 * which allows for specifying the required type once only.
	 * @param requiredType the type that each result object is expected to match
	 */
	public RowColumnRowMapper(Class<T> requiredType) {
	...
	public T mapRow(ResultSet rs, int rowNum) throws SQLException {
		Object result;
		try {
			result = this.requiredType.newInstance();
			if(result != null){
				ResultSetMetaData rsmd = rs.getMetaData();
				for(int i =1; i<=rsmd.getColumnCount(); i++){
					String name = rsmd.getColumnName(i);
					Field fieid = this.requiredType.getDeclaredField(name);
					Object value = getColumnValue(rs, i, fieid.getType());
					if(value != null){
						name = name.substring(0,1).toUpperCase() + name.substring(1, name.length());
						Method method = this.requiredType.getMethod("set" + name , fieid.getType());
	...
	/**
	 * Static factory method to create a new {@code ResultRowMapper}
	 * (with the required type specified only once).
	 * @param requiredType the type that each result object is expected to match
	 * @since 4.1
	 */
	public static <T> RowColumnRowMapper<T> newInstance(Class<T> requiredType) {
		return new RowColumnRowMapper<T>(requiredType);
	}
